...
hisTruthFilePath='/g/data/fu5/deg581/EAC_2yr_truthRun_ACCESS_baseline_dt100/output/' # Perturbed file settings
...
ds=loadOverlappedNetcdfFileList(hisTruthFilePath,prefixTruth,filelist=filelistTruth,overlapDays=7)
...
ds["temp"] = ds.temp.rolling(ocean_time=smoothWindow, center=True).mean()
ds["salt"] = ds.salt.rolling(ocean_time=smoothWindow, center=True).mean()
ds["zeta"] = ds.zeta.rolling(ocean_time=smoothWindow, center=True).mean()
ds["w"] = ds.w.rolling(ocean_time=smoothWindow, center=True).mean()
...
ds.load().to_netcdf(path=outputPath+outputPrefix+expNames+outputSuffix)
ds.close()
print('finished saving')

# ds.to_netcdf('/g/data/fu5/deg581/OSSE_analysis/data/proc/output_ds_8005_8041.nc')
